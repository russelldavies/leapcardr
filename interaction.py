...
class Account(object):
    ...
    def list_cards(self):
        ...
        the form 'card name: card id'.
        """
        url = BASE_URL + "/en/SelfServices/CardServices/CardOverView.aspx"

        self.br.open(url)
        soup = BeautifulSoup(self.br.response().read())
        card_tags = soup.find('select').find_all('option')
        for tag in card_tags:
            tag.has_attr('selected')
        ...
        overview_vals = [tag.contents[2].strip() for tag in
                soup.find(id=re.compile('CardDetails')).find_all('li')]
        overview_keys = ('number', 'label', 'type', 'status', 'credit_status',
                'auto_topup', 'init_date', 'expiry_date', 'balance')
        card_overview = zip(overview_keys, overview_vals)


        return {tag['value']: tag.text for tag in card_tags}
